...
- [x] 人類使用者可在 Settings provision 自己管理的 Agent User
- [x] Agent User 具有獨立 identity 與 ownership，不與管理者共用資料擁有權
- [x] 每個 Agent User 同時只能有一個 active API Token
- [x] 產生新 Token 時立即撤銷舊 Token
- [x] Token 預設不自動過期，並可由管理者撤銷
- [x] Settings 顯示 Token created、last used 與 last successful request time
- [x] 只有 managing human User 可管理該 Agent User 與 Token
- [x] Agent User 可透過公開 exchange flow 使用 Token 驗證
- [x] 覆蓋 rotation、revocation、ownership、replay 與錯誤回應
