...
- [x] 主導覽為 Today、Review、Watchlist、Calendar、Tools、Settings
- [x] 移除或封閉舊 Diary、Trade Draft、Partner Compare、Educational Articles、standalone Research Timeline、standalone Diary Reminder 與舊 Performance surfaces
- [x] 刪除只服務永久移除能力的 backend services、routes、contracts、migrations wiring 與 frontend code
- [x] 保留 Identity、Journal、Market Data、Tool 與 Edge 第一版 active boundaries
- [x] 保留匿名 calculators、Calculator Presets 與 Calculation Snapshots
- [x] Average Cost 與 Profit/Loss 維持 standalone，Action Decision Draft 仍 deferred
- [x] English 與 Traditional Chinese core workflows 使用現有 typed i18n architecture
- [x] mobile-first core flows 符合 PRODUCT accessibility requirements
- [x] full-stack Edge release smoke 串起 Human、Observation、Expectation、Review、Watchlist、Daily Close、Agent User、Access Grant、incremental deletion 與 removed routes
- [x] migration verification 覆蓋 ownership、one-token invariant、grant constraints、symbol history、raw／adjusted published views 與 content-free event constraints
- [x] CI、frontend tests、service integration tests、release smoke 與 migration safety checks 全部通過

完成證據（2026-07-29）：全套 .NET 103 tests、frontend 61 tests、41 migrations、166 contract operations、Docker builds、role isolation 與 isolated full-stack release smoke 全部通過。
