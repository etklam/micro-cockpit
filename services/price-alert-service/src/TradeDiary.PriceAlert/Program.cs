...
static string? Validate(PriceAlertWrite x){var evaluation=PriceAlertEngine.EvaluationPriceOrDefault(x.EvaluationPrice);if(string.IsNullOrWhiteSpace(x.Symbol)||x.Symbol.Trim().Length>24)return "invalid_symbol";if(x.ConditionType is not ("above" or "below" or "percent_change" or "ma_crossing"))return "invalid_condition_type";if(evaluation is not ("open" or "close")||x.ConditionType is ("percent_change" or "ma_crossing")&&evaluation!="close")return "invalid_evaluation_price";if(x.ConditionType is "above" or "below" && x.Threshold<0)return "invalid_threshold";if(x.ConditionType=="percent_change"&&x.Threshold==0)return "invalid_threshold";if(x.ConditionType=="ma_crossing"&&(x.LookbackDays is <2 or >250||x.Direction is not ("above" or "below")))return "invalid_moving_average";return null;}
static void Add(NpgsqlCommand c,Guid id,Guid user,string symbol,PriceAlertWrite x,decimal? baseline){c.Parameters.AddWithValue(id);c.Parameters.AddWithValue(user);c.Parameters.AddWithValue(symbol);c.Parameters.AddWithValue(x.ConditionType);c.Parameters.AddWithValue(x.Threshold);c.Parameters.AddWithValue((object?)x.LookbackDays??DBNull.Value);c.Parameters.AddWithValue((object?)x.Direction??DBNull.Value);c.Parameters.AddWithValue((object?)baseline??DBNull.Value);c.Parameters.AddWithValue(PriceAlertEngine.EvaluationPriceOrDefault(x.EvaluationPrice));}
static PriceAlertResponse Read(NpgsqlDataReader r)=>new(r.GetGuid(0),r.GetString(1),r.GetString(2),r.GetDecimal(3),r.IsDBNull(4)?null:r.GetInt32(4),r.IsDBNull(5)?null:r.GetString(5),r.GetString(6),r.GetString(7),r.IsDBNull(8)?null:r.GetDecimal(8),r.IsDBNull(9)?null:r.GetFieldValue<DateOnly>(9),r.GetDateTime(10),r.GetDateTime(11));
static async Task<IResult> SetStatus(Guid id,string status,HttpRequest request,NpgsqlDataSource db,bool requireHealthy){if(!UserId(request,out var user))return Results.Unauthorized();await using var connection=await db.OpenConnectionAsync();if(requireHealthy&&!await PriceAlertEngine.MarketHealthy(connection))return Results.Json(new { error="market_unhealthy" },statusCode:503);await using var cmd=new NpgsqlCommand("UPDATE price_alert.alerts SET status=$3,updated_at=now() WHERE id=$1 AND user_id=$2",connection);cmd.Parameters.AddWithValue(id);cmd.Parameters.AddWithValue(user);cmd.Parameters.AddWithValue(status);return await cmd.ExecuteNonQueryAsync()==0?Results.Problem("not_found",statusCode:404):Results.NoContent();}
record PriceAlertWrite(string Symbol,string ConditionType,decimal Threshold,int? LookbackDays,string? Direction,string? EvaluationPrice=null);
record PriceAlertResponse(Guid Id,string Symbol,string ConditionType,decimal Threshold,int? LookbackDays,string? Direction,string EvaluationPrice,string Status,decimal? BaselineClose,DateOnly? LastEvaluatedDate,DateTime CreatedAt,DateTime UpdatedAt);
record TriggerResponse(Guid Id,DateOnly TradingDate,decimal ObservedClose,decimal ObservedPrice,string PriceType,DateTime TriggeredAt,DateTime? DismissedAt);
...
public static class PriceAlertEngine
{
    ...
    public static async Task<int> Evaluate(NpgsqlDataSource db, int limit)
    {
        ...
        await using var claim = new NpgsqlCommand("SELECT id,symbol,condition_type,threshold,lookback_days,direction,baseline_close,evaluation_price,last_evaluated_date FROM price_alert.alerts WHERE status='active' ORDER BY created_at FOR UPDATE SKIP LOCKED LIMIT $1", connection, transaction);
