...
app.MapGet("/internal/stocks/{symbol}", async Task<IResult> (string symbol, NpgsqlDataSource db) =>
{
    ...
    return await reader.ReadAsync() ? Results.Ok(ReadStock(reader)) : Results.NotFound();
});
...
app.MapPost("/internal/stocks", async Task<IResult> (StockWrite input, NpgsqlDataSource db) =>
{
    ...
    if (symbol.Length is 0 or > 20 || name.Length is 0 or > 200) return Results.BadRequest(new { error = "symbol_and_name_required" });
    if (!string.Equals(input.AssetType ?? "stock", "stock", StringComparison.OrdinalIgnoreCase)) return Results.BadRequest(new { error = "stocks_only" });
    var id = Guid.NewGuid();
    ...
    catch (PostgresException ex) when (ex.SqlState == PostgresErrorCodes.UniqueViolation) { return Results.Conflict(new { error = "symbol_exists" }); }
});
...
app.MapPost("/internal/watchlist/{stockId:guid}", async Task<IResult> (Guid stockId, HttpRequest request, NpgsqlDataSource db) =>
{
    ...
    if (result is not null) return Results.Created($"/internal/watchlist/{stockId}", new { stockId });
    ...
    return (bool)(await exists.ExecuteScalarAsync() ?? false) ? Results.NoContent() : Results.NotFound();
});
...
app.MapGet("/internal/stocks/{stockId:guid}/note", async Task<IResult> (Guid stockId, HttpRequest request, NpgsqlDataSource db) =>
{
    ...
    return await reader.ReadAsync() ? Results.Ok(ReadNote(reader)) : Results.NotFound();
});
...
app.MapPut("/internal/stocks/{stockId:guid}/note", async Task<IResult> (Guid stockId, NoteWrite input, HttpRequest request, NpgsqlDataSource db) =>
{
    ...
    await using var reader = await command.ExecuteReaderAsync(); return await reader.ReadAsync() ? Results.Ok(ReadNote(reader)) : Results.NotFound();
});
...
app.MapGet("/internal/stocks/{stockId:guid}/timeline", async Task<IResult> (Guid stockId, HttpRequest request, NpgsqlDataSource db) =>
{
    ...
    while (await reader.ReadAsync()) items.Add(ReadTimeline(reader)); return Results.Ok(new { items });
});
...
app.MapGet("/internal/timeline/{id:guid}", async Task<IResult> (Guid id, HttpRequest request, NpgsqlDataSource db) =>
{
    ...
    return await reader.ReadAsync() ? Results.Ok(ReadTimeline(reader)) : Results.NotFound();
});
...
app.MapPost("/internal/timeline/{originalId:guid}/corrections", async Task<IResult> (Guid originalId, TimelineWrite input, HttpRequest request, NpgsqlDataSource db) =>
{
    ...
    return stockId is Guid id ? await AppendTimeline(db, userId, id, originalId, input) : Results.NotFound();
});
...
static async Task<IResult> AppendTimeline(NpgsqlDataSource db, Guid userId, Guid stockId, Guid? correctionOfId, TimelineWrite input)
{
    ...
    if (sourceType.Length == 0 || title.Length == 0 || content.Length == 0) return Results.BadRequest(new { error = "source_type_title_and_content_required" });
    ...
    await using var reader = await command.ExecuteReaderAsync(); return await reader.ReadAsync() ? Results.Created($"/internal/timeline/{id}", ReadTimeline(reader)) : Results.NotFound();
}
