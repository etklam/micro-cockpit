...
const monitor = {
  universe: { id: universes.items[0].id, code: 'SECTORS', name: 'US sectors' },
  ...
  etfs: [
    { symbol: 'XLE', label: 'Energy', sector: 'Energy', close: 88.1, return2w: -2.5, return1m: 1.1, return3m: 3.2, rank2w: 2, percentile2w: 0, aboveMa20: false, aboveMa50: true, aboveMa200: null, status: 'ok' },
    { symbol: 'XLK', label: 'Technology', sector: 'Technology', close: 240.5, return2w: 4.5, return1m: 8.1, return3m: 12.2, rank2w: 1, percentile2w: 1, aboveMa20: true, aboveMa50: true, aboveMa200: true, status: 'ok' },
  ],
}

function handlers(options?: { universes?: unknown; monitor?: unknown; monitorStatus?: number; onMonitor?: (url: URL) => void }) {
  return [
    ...
    http.get('/api/app/rotation/monitor', ({ request }) => {
      options?.onMonitor?.(new URL(request.url))
      return HttpResponse.json(options?.monitor ?? monitor, { status: options?.monitorStatus ?? 200 })
    }),
  ]
}
...
test('rotation dashboard renders existing snapshot fields in backend rank order', async () => {
  server.use(...handlers())
  renderRotation()
  ...
  expect(within(rows[1]).getByText('XLK')).toBeInTheDocument()
  expect(within(rows[2]).getByText('XLE')).toBeInTheDocument()
})
...
test('null performance values sort last', async () => {
  const withNull = { ...monitor, etfs: [...monitor.etfs, { ...monitor.etfs[0], symbol: 'XLF', label: 'Financials', sector: 'Financials', rank2w: 3, return2w: null }] }
  server.use(...handlers({ monitor: withNull }))
  renderRotation('/rotation?universe=SECTORS&scope=sector&sort=return2w&direction=desc')

  const rows = await screen.findAllByRole('row')
  expect(within(rows[1]).getByText('XLK')).toBeInTheDocument()
  expect(within(rows[3]).getByText('XLF')).toBeInTheDocument()
})
