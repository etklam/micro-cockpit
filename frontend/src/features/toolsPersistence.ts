...
export type SavedCalculation = { id: string; toolType: ToolId; schemaVersion: number; inputs: Record<string, number | string>; output: Record<string, number>; currency: string; symbol: string | null; note: string | null; createdAt: string }
...
export const saveCalculation = (body: { toolType: ToolId; inputs: Record<string, number | string>; currency: string; symbol: string | null; note: string | null }, key: string) => G.request<SavedCalculation>('/api/app/saved-calculations', { method: 'POST', headers: { 'Idempotency-Key': key }, body: JSON.stringify(body) })
