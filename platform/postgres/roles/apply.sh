...
required='MIGRATOR_DB_PASSWORD IDENTITY_DB_PASSWORD JOURNAL_DB_PASSWORD MARKET_DATA_DB_PASSWORD TOOL_DB_PASSWORD'
...
psql -v ON_ERROR_STOP=1 -f /roles/001_bootstrap_roles.sql
{
  ...
  for mapping in \
    ...
    journal_service:JOURNAL_DB_PASSWORD market_data_service:MARKET_DATA_DB_PASSWORD \
