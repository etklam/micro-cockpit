...
dropped = set(re.findall(r"DROP\s+SCHEMA\s+(?:IF\s+EXISTS\s+)?([a-z_][a-z0-9_]*)\s+CASCADE", all_sql, re.I))
dropped_lower = {name.lower() for name in dropped}
tables = [
    (p.name, schema, table)
    for p in sql_files
    for schema, table in re.findall(
        r"CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?([a-z_][a-z0-9_]*)\.([a-z_][a-z0-9_]*)",
        p.read_text(),
        re.I,
    )
    if schema.lower() not in dropped_lower
]
errors = []
