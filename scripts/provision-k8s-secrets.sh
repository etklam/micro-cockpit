...
required='POSTGRES_PASSWORD MIGRATOR_DB_PASSWORD IDENTITY_DB_PASSWORD JOURNAL_DB_PASSWORD MARKET_DATA_DB_PASSWORD TOOL_DB_PASSWORD LOCAL_REGISTRATION_KEY INTERNAL_SERVICE_KEY'
...
for mapping in \
  ...
  TOOL:tool_service:TOOL_DB_PASSWORD
do
  prefix=${mapping%%:*}
  rest=${mapping#*:}
  role=${rest%%:*}
