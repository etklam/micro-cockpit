...
if len(contracts) != 5:
    raise SystemExit(f"expected 5 OpenAPI documents, found {len(contracts)}")
...
expected = {
    ...
    ("journal-service.openapi.json", "/internal/quick-observations", "post"): {"bearerAuth"},
    ("edge-api.openapi.json", "/api/auth/login", "post"): set(),
    ("edge-api.openapi.json", "/api/auth/api-key/token", "post"): set(),
    ("edge-api.openapi.json", "/api/app/market-observations", "get"): {"bearerAuth"},
}
