...
public sealed class ToolWorkflowApiTests
{
    ...
    public async Task Presets_and_saved_calculations_are_validated_recalculated_idempotent_and_user_scoped()
    {
        ...
        var export=await client.GetFromJsonAsync<JsonElement>("/internal/account-export");
        Assert.Single(export.GetProperty("presets").EnumerateArray());
        Assert.Single(export.GetProperty("savedCalculations").EnumerateArray());
        Assert.Empty((await otherClient.GetFromJsonAsync<JsonElement>("/internal/account-export")).GetProperty("presets").EnumerateArray());
        Assert.Equal(HttpStatusCode.NoContent,(await client.DeleteAsync("/internal/account-data")).StatusCode);
        Assert.Empty((await client.GetFromJsonAsync<JsonElement>("/internal/tool-presets")).GetProperty("items").EnumerateArray());
        Assert.Empty((await client.GetFromJsonAsync<JsonElement>("/internal/saved-calculations")).GetProperty("items").EnumerateArray());
    }
