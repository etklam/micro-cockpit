...
test "$(curl -sS -o /dev/null -w '%{http_code}' -H 'X-Service-Key: local-service-key' -H 'Content-Type: application/json' -d "$payload" "$reminder/internal/events/diary-deleted")" = 204
