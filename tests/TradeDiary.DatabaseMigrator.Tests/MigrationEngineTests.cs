...
public sealed class MigrationEngineTests : IAsyncLifetime
{
    ...
    public async Task FreshDatabaseIsTransactionalIdempotentAndAllRuntimeRolesAreSeparated()
    {
        ...
        Assert.Equal(19L, await Scalar<long>("SELECT count(*) FROM platform_migrations.schema_history"));
    ...
    public async Task FailureRollsBackAndHistoryProtectionsRejectDrift()
    {
        ...
        Assert.Equal(0L, await Scalar<long>("SELECT count(*) FROM platform_migrations.schema_history WHERE migration_id='0020'"));
        File.Delete(Path.Combine(fixture, "0020_failure.sql"));
