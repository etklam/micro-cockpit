...
public sealed class MigrationEngineTests : IAsyncLifetime
{
    ...
    public async Task FreshDatabaseIsTransactionalIdempotentAndAllRuntimeRolesAreSeparated()
    {
        ...
        Assert.Equal(42L, await Scalar<long>("SELECT count(*) FROM platform_migrations.schema_history"));
    ...
    public async Task FailureRollsBackAndHistoryProtectionsRejectDrift()
    {
        ...
        Assert.Equal(0L, await Scalar<long>("SELECT count(*) FROM platform_migrations.schema_history WHERE migration_id='0043'"));
        File.Delete(Path.Combine(fixture, "0043_failure.sql"));
